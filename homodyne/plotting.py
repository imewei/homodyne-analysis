...
from io_utils import ensure_dir, save_fig
...
def get_plot_config(config: Optional[Dict] = None) -> Dict[str, Any]:
    ...
    if config and "output_settings" in config and "plotting" in config["output_settings"]:
        plot_config = {**default_plot_config, **config["output_settings"]["plotting"]}
    else:
...
def plot_c2_heatmaps(exp: np.ndarray, theory: np.ndarray, phi_angles: np.ndarray, 
                     outdir: Union[str, Path], config: Optional[Dict] = None,
                     time_lags: Optional[np.ndarray] = None,
                     delay_times: Optional[np.ndarray] = None) -> bool:
    """
    ...
    if len(phi_angles) != exp.shape[0]:
        logger.error(f"Number of angles ({len(phi_angles)}) doesn't match data shape ({exp.shape[0]})")
        return False
    ...
    for i, phi in enumerate(phi_angles):
        try:
            ...
            im1 = ax1.imshow(exp[i], aspect='auto', origin='lower', 
                           extent=[delay_times[0], delay_times[-1], time_lags[0], time_lags[-1]],
                           cmap='viridis')
            ...
            im2 = ax2.imshow(theory[i], aspect='auto', origin='lower',
                           extent=[delay_times[0], delay_times[-1], time_lags[0], time_lags[-1]],
                           cmap='viridis')
            ...
            im3 = ax3.imshow(residuals[i], aspect='auto', origin='lower',
                           extent=[delay_times[0], delay_times[-1], time_lags[0], time_lags[-1]],
                           cmap='RdBu_r')
            ...
            if save_fig(fig, filepath, dpi=plot_config["dpi"], format=plot_config["plot_format"]):
                success_count += 1
    ...
    logger.info(f"Successfully created {success_count}/{len(phi_angles)} C2 heatmap plots")
    return success_count == len(phi_angles)


def plot_parameter_evolution(best_params: Dict[str, float], bounds: List[Dict], 
                           outdir: Union[str, Path], config: Optional[Dict] = None,
                           initial_params: Optional[Dict[str, float]] = None,
                           optimization_history: Optional[List[Dict]] = None) -> bool:
    """
    ...
    try:
        ...
        initial_values = [initial_params.get(name, 0) if initial_params else 0 for name in param_names]
        
        # Create figure with two subplots
        fig, (ax1, ax2) = plt.subplots(2, 1, figsize=(plot_config["figure_size"][0], 
        ...
        # Create bars
        bars1 = ax1.bar(x_pos - width, normalized_initial, width, label='Initial', alpha=0.7, color='lightblue')
        bars2 = ax1.bar(x_pos, normalized_best, width, label='Best Fit', alpha=0.7, color='darkblue')
        bars3 = ax1.bar(x_pos + width, normalized_lower, width, label='Lower Bound', alpha=0.5, color='red')
        bars4 = ax1.bar(x_pos + 1.5*width, normalized_upper, width, label='Upper Bound', alpha=0.5, color='green')
        ...
        def add_value_labels(bars, values):
            for bar, value in zip(bars, values):
                ...
                ax1.annotate(f'{value:.2e}' if abs(value) < 1e-3 or abs(value) > 1e3 else f'{value:.3f}',
                           xy=(bar.get_x() + bar.get_width() / 2, height),
                           xytext=(0, 3),  # 3 points vertical offset
                           textcoords="offset points",
                           ha='center', va='bottom', fontsize=8, rotation=90)
        
        add_value_labels(bars2, best_values)  # Only label best fit values to avoid clutter
        
        # Plot 2: Optimization history (if available)
        if optimization_history:
            iterations = range(len(optimization_history))
            chi_squared = [hist.get('chi_squared', np.nan) for hist in optimization_history]
            
            ax2.semilogy(iterations, chi_squared, 'b-', marker='o', markersize=3, linewidth=1.5)
            ...
            if chi_squared and not np.isnan(chi_squared[-1]):
                ax2.text(0.98, 0.95, f'Final χ² = {chi_squared[-1]:.6f}', 
            ...
            im = ax2.imshow(np.corrcoef(param_array.T), cmap='RdBu_r', vmin=-1, vmax=1, aspect='auto')
            ...
            for i in range(len(param_names)):
                ax2.text(i, i, f'{best_values[i]:.3e}', ha='center', va='center',
                        color='white' if abs(best_values[i]) > np.mean(np.abs(best_values)) else 'black',
        ...
        success = save_fig(fig, filepath, dpi=plot_config["dpi"], format=plot_config["plot_format"])
        plt.close(fig)
...
def plot_mcmc_corner(trace_data: Any, outdir: Union[str, Path], 
                    config: Optional[Dict] = None,
                    param_names: Optional[List[str]] = None,
                    param_units: Optional[List[str]] = None,
                    title_prefix: str = "MCMC") -> bool:
    """
    ...
    try:
        ...
        if CORNER_AVAILABLE:
            ...
            fig = corner.corner(stacked_samples, 
                              labels=[f"{name}\n[{unit}]" if param_names and param_units 
                                     else f"Param {i}" for i, (name, unit) in 
                                     enumerate(zip(param_names or [], param_units or []))],
                              show_titles=True,
            ...
            axes = az.plot_pair(stacked_samples, kind='kde', marginals=True, 
                              figsize=plot_config["figure_size"])
            fig = axes.ravel()[0].figure
        
        # Add title
        fig.suptitle(f'{title_prefix} Posterior Distribution Corner Plot', 
                    fontsize=16, y=0.98)
...
def plot_diagnostic_summary(results: Dict[str, Any], outdir: Union[str, Path],
    ...
    try:
        ...
        fig = plt.figure(figsize=(plot_config["figure_size"][0] * 1.5, 
                                plot_config["figure_size"][1] * 1.2))
        ...
        if chi2_values:
            bars = ax1.bar(methods, chi2_values, alpha=0.7, color=['C0', 'C1', 'C2', 'C3'][:len(methods)])
        ...
        if 'mcmc_diagnostics' in results and ARVIZ_AVAILABLE:
            ...
            if 'r_hat' in diagnostics:
                ...
                ax3.axvline(x=1.1, color='red', linestyle='--', alpha=0.7, label='R̂ = 1.1')
        ...
        if 'residuals' in results:
            ...
            if isinstance(residuals, np.ndarray):
                ...
                ax4.hist(flat_residuals, bins=50, alpha=0.7, density=True, color='skyblue')
                ...
                ax4.plot(x, (1/(sigma * np.sqrt(2 * np.pi))) * np.exp(-0.5 * ((x - mu)/sigma)**2), 
                        'r-', linewidth=2, label=f'Normal(μ={mu:.3e}, σ={sigma:.3e})')
...
def create_all_plots(results: Dict[str, Any], outdir: Union[str, Path],
    ...
    if all(key in results for key in ['experimental_data', 'theoretical_data', 'phi_angles']):
...
if __name__ == "__main__":
    ...
    np.random.seed(42)
    exp_data = 1 + 0.5 * np.random.exponential(1, (n_angles, n_time_lags, n_delay_times))
    ...
    with tempfile.TemporaryDirectory() as tmp_dir:
        ...
        # Test C2 heatmaps
        success1 = plot_c2_heatmaps(exp_data, theory_data, phi_angles, tmp_dir, test_config)
