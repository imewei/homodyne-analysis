...
class VisualizationOptimizer:
    ...
    def _create_matplotlib_heatmap(
        ...
        **kwargs,
        ...
        """Create correlation heatmap using matplotlib with optimizations."""
        results = {"backend": "matplotlib", "plots_created": [], "performance": {}}
        ...
        if self.quality == "adaptive":
            ...
            dpi, interpolation = quality_settings.get(self.quality, (200, "bilinear"))
        ...
        for i, phi in enumerate(phi_angles):
            ...
            exp_slice = self.optimize_data_for_plotting(exp_data[i])
            theory_slice = self.optimize_data_for_plotting(theory_data[i])
            residual_slice = exp_slice - theory_slice

            # Common colormap settings
            vmin = min(np.min(exp_slice), np.min(theory_slice))
            vmax = max(np.max(exp_slice), np.max(theory_slice))
            ...
            axes[0].set_title(f"Experimental C2 (φ = {phi:.1f}°)")
            axes[0].set_xlabel("t₁")
            axes[0].set_ylabel("t₂")
            ...
            axes[1].set_title(f"Theoretical C2 (φ = {phi:.1f}°)")
            axes[1].set_xlabel("t₁")
            axes[1].set_ylabel("t₂")
            ...
            plt.colorbar(im1, ax=axes[0], label="C2", shrink=0.8)
            plt.colorbar(im2, ax=axes[1], label="C2", shrink=0.8)
            plt.colorbar(im3, ax=axes[2], label="Residual", shrink=0.8)
            ...
            if output_path:
                angle_filename = (
                    output_path.parent
                    / f"{output_path.stem}_phi_{phi:.1f}deg{output_path.suffix}"
                )
                plt.savefig(
                    angle_filename,
                    dpi=dpi,
                ...
                results["plots_created"].append(str(angle_filename))
        ...
        # Record performance
        results["performance"] = {
            "render_time": time.time() - time.time(),
            "dpi": dpi,
            "interpolation": interpolation,
            "plots_count": len(phi_angles),
        }
    ...
    def _create_plotly_heatmap(
        ...
        **kwargs,
        ...
        if not PLOTLY_AVAILABLE:
            raise ImportError("Plotly not available for interactive plotting")

        results = {"backend": "plotly", "plots_created": [], "performance": {}}
        ...
        for i, phi in enumerate(phi_angles):
            ...
            fig = make_subplots(
                ...
                cols=3,
                subplot_titles=(
                    f"Experimental C2 (φ = {phi:.1f}°)",
                    f"Theoretical C2 (φ = {phi:.1f}°)",
                    f"Residuals (φ = {phi:.1f}°)",
                ),
                horizontal_spacing=0.1,
            )

            # Optimize data for interactive plotting
            exp_slice = self.optimize_data_for_plotting(
                exp_data[i], target_points=50000
            )
            theory_slice = self.optimize_data_for_plotting(
                theory_data[i], target_points=50000
            )
            residual_slice = exp_slice - theory_slice

            # Add heatmaps
            fig.add_trace(
                go.Heatmap(z=exp_slice, colorscale="Viridis", name="Experimental"),
                row=1,
                col=1,
            )

            fig.add_trace(
                go.Heatmap(z=theory_slice, colorscale="Viridis", name="Theoretical"),
                row=1,
                col=2,
            )

            fig.add_trace(
                go.Heatmap(z=residual_slice, colorscale="RdBu", name="Residuals"),
                row=1,
                col=3,
            )
            ...
            # Update layout
            fig.update_layout(
                title=f"{title} - φ = {phi:.1f}°", height=500, showlegend=False
            )

            # Save interactive plot
            if output_path:
                angle_filename = (
                    output_path.parent / f"{output_path.stem}_phi_{phi:.1f}deg.html"
                )
                fig.write_html(str(angle_filename))
                results["plots_created"].append(str(angle_filename))
    ...
    def _create_matplotlib_evolution(
        self,
        parameter_history: dict[str, list[float]],
        output_path: Path | None,
        title: str,
    ) -> dict[str, Any]:
        """Create parameter evolution plot using matplotlib."""
        fig, ax = plt.subplots(figsize=(12, 8))

        colors = self.palettes["colorblind"]

        for i, (param_name, values) in enumerate(parameter_history.items()):
            color = colors[i % len(colors)]
            iterations = list(range(len(values)))

            ax.plot(iterations, values, label=param_name, color=color, linewidth=2)
            ax.scatter(
                iterations[:: max(1, len(iterations) // 20)],
                [
                    values[j]
                    for j in range(0, len(values), max(1, len(iterations) // 20))
                ],
                color=color,
                s=30,
                alpha=0.7,
        ...
        result = {
            "backend": "matplotlib",
            "plot_created": str(output_path) if output_path else None,
        }
        plt.close(fig)

        return result

    def _create_plotly_evolution(
        self,
        parameter_history: dict[str, list[float]],
        output_path: Path | None,
        title: str,
    ) -> dict[str, Any]:
        """Create interactive parameter evolution plot using Plotly."""
        if not PLOTLY_AVAILABLE:
            raise ImportError("Plotly not available")
        ...
        return {
            "backend": "plotly",
            "plot_created": str(html_path) if output_path else None,
        }
    ...
    def _create_plotly_dashboard(
        self, analysis_results: dict[str, Any], output_path: Path | None
    ) -> dict[str, Any]:
        ...
        # Method comparison (Chi-squared values)
        method_results = analysis_results.get("method_results", {})
        methods = list(method_results.keys())
        chi2_values = [method_results[m].get("chi_squared", 0) for m in methods]

        fig.add_trace(
            go.Bar(x=methods, y=chi2_values, name="Chi-squared"), row=1, col=1
        )

        # Add other dashboard components...
        fig.update_layout(
            height=800, showlegend=False, title="Analysis Performance Dashboard"
        )
        ...
        return {
            "backend": "plotly",
            "dashboard_created": str(dashboard_path) if output_path else None,
        }

    def _create_matplotlib_dashboard(
        self, analysis_results: dict[str, Any], output_path: Path | None
    ) -> dict[str, Any]:
        """Create static performance dashboard using matplotlib."""
        fig, axes = plt.subplots(2, 2, figsize=(16, 12))
        fig.suptitle("Analysis Performance Dashboard", fontsize=16)
        ...
        axes[0, 0].bar(methods, chi2_values)
        axes[0, 0].set_title("Method Comparison (Chi-squared)")
        ...
        if output_path:
            dashboard_path = output_path.with_name(f"{output_path.stem}_dashboard.png")
