...
def configure_jax_backend(force_cpu: bool = False, gpu_memory_fraction: float = 0.8) -> Dict[str, Any]:
    """
    ...
    try:
        ...
        if not force_cpu and len(gpu_devices) > 0:
            ...
            logger.info(f"✅ JAX configured for GPU: {len(gpu_devices)} GPU(s) available")
...
def run_gpu_mcmc_analysis(
    ...
    **kwargs
    ...
    logger.info(f"🚀 Initializing NumPyro {jax_config['backend']} backend (PyMC isolated)")
