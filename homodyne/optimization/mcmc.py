...
class MCMCSampler:
    ...
    def _build_bayesian_model_optimized(
        ...
        if filter_angles_for_optimization:
            ...
            if optimization_indices:
                ...
                print(f"   MCMC angle filtering: using {len(optimization_indices)}/{n_angles} angles")
                print(f"   Optimization angles: {[f'{angle:.1f}°' for angle in phi_angles_filtered]}")
                logger.info(f"MCMC using filtered angles: {len(optimization_indices)}/{n_angles} angles")
            else:
                print("   Warning: No angles found in optimization ranges [-10°, 10°] and [170°, 190°]")
        ...
        with pm.Model() as model:
            ...
            bounds = self.config.get("parameter_space", {}).get("bounds", [])  # noqa: F841
            ...
            phi_angles_shared = shared(phi_angles.astype(dtype), name="phi_angles")  # noqa: F841
            ...
            gamma_positive = pm.Deterministic("gamma_positive", gamma_dot_t0 > 0)  # noqa: F841
    ...
    def _run_mcmc_nuts_optimized(
        ...
        model = self._build_bayesian_model_optimized(
            c2_experimental, phi_angles, filter_angles_for_optimization=filter_angles_for_optimization
        )
        ...
        for var_name in param_names:
            if hasattr(trace, 'posterior') and var_name in trace.posterior:  # type: ignore[attr-defined]
                posterior_means[var_name] = float(trace.posterior[var_name].mean())  # type: ignore[attr-defined]
    ...
    def run_mcmc_analysis(
        ...
        assert c2_experimental is not None and phi_angles is not None, "Failed to load experimental data"
        ...
        if filter_angles_for_optimization is None:
            ...
            if hasattr(self.core, 'config_manager') and self.core.config_manager:
                filter_angles_for_optimization = self.core.config_manager.is_angle_filtering_enabled()
            else:
                # Default to True for backward compatibility
                filter_angles_for_optimization = True
        
        # Ensure filter_angles_for_optimization is a boolean
        assert isinstance(filter_angles_for_optimization, bool), "filter_angles_for_optimization must be a boolean"
            
        # Run MCMC sampling with angle filtering
        results = self._run_mcmc_nuts_optimized(
            c2_experimental, phi_angles, mcmc_config, filter_angles_for_optimization
        )
    ...
    def compute_convergence_diagnostics(self, trace) -> Dict[str, Any]:
        ...
        if not PYMC_AVAILABLE or az is None:
            ...
            return {"converged": True, "note": "Diagnostics unavailable - arviz not installed"}
        ...
        try:
            ...
            try:
                max_rhat = float(rhat.to_array().max()) if hasattr(rhat, 'to_array') else float(np.max(rhat))  # type: ignore[attr-defined]
            except (AttributeError, TypeError):
                max_rhat = 1.0
            
            try:
                min_ess = float(ess.to_array().min()) if hasattr(ess, 'to_array') else float(np.min(ess))
            except (AttributeError, TypeError):
                min_ess = 1000.0
    ...
    def extract_posterior_statistics(self, trace) -> Dict[str, Any]:
        ...
        try:
            ...
            for param in param_names:
                if hasattr(trace, 'posterior') and param in trace.posterior:  # type: ignore[attr-defined]
                    samples = trace.posterior[param].values.flatten()  # type: ignore[attr-defined]
            ...
            return {
                ...
                "total_samples": (len(trace.posterior.chain) * len(trace.posterior.draw)) if hasattr(trace, 'posterior') and hasattr(trace.posterior, 'chain') and hasattr(trace.posterior, 'draw') else 0,  # type: ignore[attr-defined]
            }
    ...
    def generate_posterior_samples(self, n_samples: int = 1000) -> Optional[np.ndarray]:
        ...
        try:
            ...
            for param in param_names:
                if hasattr(self.mcmc_trace, 'posterior') and param in self.mcmc_trace.posterior:  # type: ignore[attr-defined]
                    param_samples = self.mcmc_trace.posterior[param].values.flatten()  # type: ignore[attr-defined]
                    # Randomly subsample if more samples available than requested
                    if len(param_samples) > n_samples:
                        indices = np.random.choice(len(param_samples), n_samples, replace=False)
    ...
    def assess_chain_mixing(self, trace) -> Dict[str, Any]:
        ...
        try:
            ...
            if hasattr(trace, 'sample_stats') and 'diverging' in trace.sample_stats:
                ...
                divergent_fraction = float(n_divergent / trace.sample_stats.diverging.size)
            else:
                n_divergent = 0
            ...
            try:
                if ess is not None:
                    trace_len = len(trace.posterior.draw) if hasattr(trace, 'posterior') else 1000  # type: ignore[attr-defined]
    ...
    def _validate_mcmc_config(self) -> None:
        ...
        if not isinstance(mcmc_chains, int) or mcmc_chains < 1:
            raise ValueError(f"mcmc_chains must be a positive integer, got {mcmc_chains}")
    ...
    def _validate_physical_parameters(self, params: np.ndarray) -> bool:
        ...
        try:
            ...
            if "D0" in param_dict and param_dict["D0"] <= 0:
                logger.warning(f"Non-physical diffusion coefficient: {param_dict['D0']}")
                return False
    ...
    def validate_model_setup(self) -> Dict[str, Any]:
        ...
        for section in required_sections:
            if section not in self.config:
                validation_results["valid"] = False
                validation_results["errors"].append(f"Missing configuration section: {section}")
    ...
    def get_model_summary(self) -> Optional[Dict[str, Any]]:
        ...
        try:
            with self.bayesian_model:
                ...
                deterministic_vars = [rv.name for rv in self.bayesian_model.deterministics]
                ...
                return {
                    ...
                    "forward_model": "Simplified" if self.config.get(
                        "performance_settings", {}
                    ).get("noise_model", {}).get("use_simple_forward_model", True) else "Full"
                }
    ...
    def get_best_params(self, stage: str = "mcmc") -> Optional[np.ndarray]:  # noqa: ARG002
        """
    ...
    def get_parameter_uncertainties(self) -> Optional[Dict[str, float]]:
        ...
        try:
            ...
            for param in param_names:
                if hasattr(self.mcmc_trace, 'posterior') and param in self.mcmc_trace.posterior:  # type: ignore[attr-defined]
                    samples = self.mcmc_trace.posterior[param].values.flatten()  # type: ignore[attr-defined]
...
if __name__ == "__main__":
    ...
    if PYMC_AVAILABLE:
        print(f"PyMC Version: {pm.__version__ if pm and hasattr(pm, '__version__') else 'unknown'}")
