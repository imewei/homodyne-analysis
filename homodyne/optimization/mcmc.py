...
class MCMCSampler:
    ...
    def __init__(self, analysis_core: Any, config: dict[str, Any]) -> None:
        ...
        # CPU-only PyMC implementation
        logger.info("Using CPU-only PyMC MCMC sampling")
        logger.info(
            "For GPU acceleration, use 'homodyne-gpu --method mcmc' with NumPyro"
        )
    ...
    def log_mcmc_progress(
        ...
        if stage in ["Sampling", "Tuning"]:
            ...
            log_parts.append(
                f"{method_display} {stage} {iteration}"
                if stage
                else f"{method_display} Iteration {iteration}"
            )
        ...
        if chain_diagnostics and diagnostics and "recommendations" in diagnostics:
            ...
            if recommendations:
                rec_msg = f"{method_display} Recommendations: " + "; ".join(
                    recommendations[:2]
    ...
    def _run_mcmc_nuts_optimized(
        ...
        completion_diagnostics = {
            ...
            "effective_draws": effective_draws,
            "sampling_efficiency": (effective_draws / mcmc_time)
            if mcmc_time > 0
            else 0,
        }
...
def create_mcmc_sampler(analysis_core: Any, config: dict[str, Any]) -> MCMCSampler:
    """
