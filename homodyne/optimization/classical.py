...
class ClassicalOptimizer:
    ...
    def run_classical_optimization_optimized(
        ...
        if is_static_mode and len(initial_parameters) > effective_param_count:
            ...
            not is_static_mode
            and len(initial_parameters) < effective_param_count
        ):
            ...
            try:
                ...
                if success and isinstance(result, optimize.OptimizeResult):
                    ...
                    if result.fun < best_chi2:
                        ...
                        best_method = (
                            method  # Track which method produced this result
                        )
                        print(
                            f"    ✓ New best: χ²_red = {
                                result.fun:.6e} ({
    ...
    def _run_gurobi_optimization(
        ...
        try:
            ...
            for i in range(n_params):
                ...
                x_minus[i] -= epsilon
                grad[i] = (
                    objective_func(x_plus) - objective_func(x_minus)
                ) / (2 * epsilon)
    ...
    def _run_robust_optimization(
        ...
        try:
            if (
                ...
                or create_robust_optimizer is None
            ):
                raise ImportError(
            ...
            if optimal_params is not None:
                ...
                result = optimize.OptimizeResult(
                    x=optimal_params,
                    fun=info.get(
                        "final_chi_squared", objective_func(optimal_params)
                    ),
                    success=True,
