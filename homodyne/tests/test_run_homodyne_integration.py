...
class TestRunHomodyneIntegration:
    ...
    def test_complete_directory_structure_integration(self, temp_directory):
        ...
        log_file.write_text("2025-08-18 10:57:33 - __main__ - INFO - Analysis completed successfully\n")
...
class TestRunHomodyneMockExecution:
    ...
    def test_plot_experimental_data_early_exit_simulation(self, temp_directory):
        ...
        def mock_run_homodyne_with_plot_experimental_data():
            ...
            summary_file.write_text("Data validation completed.\nMean g2: 1.05\nContrast: 0.02\n")
    ...
    def test_classical_method_execution_simulation(self, temp_directory):
        ...
        def mock_run_homodyne_with_classical_method():
            ...
            mock_fitted_data = mock_theory_data * 1912.0 - 1910.96  # Scaling from actual results
            mock_residuals_data = mock_exp_data - mock_fitted_data
            
            # 4. Save data files
            np.savez_compressed(classical_dir / "experimental_data.npz", data=mock_exp_data)
            np.savez_compressed(classical_dir / "fitted_data.npz", data=mock_fitted_data)
            np.savez_compressed(classical_dir / "residuals_data.npz", data=mock_residuals_data)
