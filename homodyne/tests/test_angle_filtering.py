...
class TestAngleFilteringCore:
        ...
        return {
            ...
            "parameter_space": {
                "bounds": [
                    ...
                    {"name": "D_offset", "min": -5000, "max": 5000, "type": "uniform"},
                    {"name": "gamma_dot_t0", "min": 1e-6, "max": 1.0, "type": "log-uniform"},
                    {"name": "beta", "min": -2.0, "max": 2.0, "type": "uniform"},
                    {"name": "gamma_dot_t_offset", "min": -0.1, "max": 0.1, "type": "uniform"},
    ...
    def test_angle_filtering_identification(self, test_phi_angles):
        ...
        # Verify expectations
        assert len(expected_optimization_indices) == 8  # 5 in [-10,10] + 3 in [170,190]
        assert set(expected_angles) == {-8.0, -5.0, 0.0, 5.0, 8.0, 175.0, 180.0, 185.0}
    ...
    def test_chi_squared_angle_filtering_mock(self, mock_analyzer, test_phi_angles):
        ...
        c2_experimental = np.random.rand(n_angles, 20, 20) + 1.0  # 20x20 correlation matrices
        ...
        chi2_all = mock_analyzer.calculate_chi_squared_optimized(
            ...
            method_name="Test_All", filter_angles_for_optimization=False
        )
        
        # Test with filtering (optimization angles only)
        chi2_filtered = mock_analyzer.calculate_chi_squared_optimized(
            parameters, test_phi_angles, c2_experimental,
            method_name="Test_Filtered", filter_angles_for_optimization=True
        )
    ...
    def test_detailed_chi_squared_with_filtering(self, mock_analyzer, test_phi_angles):
        ...
        result_all = mock_analyzer.calculate_chi_squared_optimized(
            parameters, test_phi_angles, c2_experimental,
            method_name="DetailedAll", return_components=True,
        ...
        result_filtered = mock_analyzer.calculate_chi_squared_optimized(
            parameters, test_phi_angles, c2_experimental,
            method_name="DetailedFiltered", return_components=True,
        ...
        assert result_all["valid"] == True
        assert result_filtered["valid"] == True
    ...
    def test_degrees_of_freedom_calculation(self, mock_analyzer, test_phi_angles):
        ...
        n_angles = len(test_phi_angles)
        c2_experimental = np.ones((n_angles, 5, 5))  # 5x5 matrices = 25 data points per angle
        parameters = np.array([1000.0, -0.1, 50.0, 0.01, -0.5, 0.001, 0.0])
        ...
        expected_data_points_filtered = len(optimization_indices) * 25  # 25 points per angle
        ...
        result_filtered = mock_analyzer.calculate_chi_squared_optimized(
            parameters, test_phi_angles, c2_experimental,
            return_components=True, filter_angles_for_optimization=True
        )
        
        # Test without filtering
        result_all = mock_analyzer.calculate_chi_squared_optimized(
            parameters, test_phi_angles, c2_experimental,
            return_components=True, filter_angles_for_optimization=False
        )
    ...
    def test_empty_optimization_angles_fallback(self, mock_analyzer):
        ...
        chi2_fallback = mock_analyzer.calculate_chi_squared_optimized(
            parameters, phi_angles_outside, c2_experimental,
            filter_angles_for_optimization=True
        )
...
class TestAngleFilteringOptimizationIntegration:
    ...
    def test_classical_optimizer_uses_angle_filtering(self):
        ...
        objective = optimizer.create_objective_function(phi_angles, c2_experimental, "Test")
        ...
        mock_analyzer.calculate_chi_squared_optimized.assert_called_once_with(
            test_params, phi_angles, c2_experimental, "Test",
            filter_angles_for_optimization=True
        )
        assert result == 10.0
    ...
    def test_bayesian_optimizer_uses_angle_filtering(self):
        ...
        try:
            from homodyne.optimization.bayesian import BayesianOptimizer, SKOPT_AVAILABLE
    ...
    def test_mcmc_sampler_uses_angle_filtering(self):
        ...
        try:
            ...
            # Test that the run_mcmc_analysis method accepts filter_angles_for_optimization parameter
            import inspect
            ...
            assert default_value is None  # Now uses None as default, resolved at runtime
            ...
            for method_name in methods_to_check:
                if hasattr(sampler, method_name):
                    ...
                    assert 'filter_angles_for_optimization' in method_sig.parameters, f"Missing parameter in {method_name}"
    ...
    def test_all_optimization_methods_use_angle_filtering(self):
        ...
        try:
            ...
            if PYMC_AVAILABLE:
                ...
                assert sig.parameters['filter_angles_for_optimization'].default is None  # Uses None, resolved at runtime
...
class TestAngleFilteringEdgeCases:
    ...
    def test_angle_filtering_ranges_definition(self):
        ...
        for angle, should_include in test_cases:
            ...
            assert is_included == should_include, f"Angle {angle}° should be {'included' if should_include else 'excluded'}"
    ...
    def test_optimization_methods_use_config_manager(self):
        ...
        try:
            ...
            if not hasattr(MCMCSampler, '_MCMCSampler__test_available'):
                ...
                assert mock_analyzer.config_manager.is_angle_filtering_enabled() == True
