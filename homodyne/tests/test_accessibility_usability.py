...
class AccessibilityTester:
    ...
    def _test_color_accessibility(self) -> List[AccessibilityIssue]:
        ...
        for cli_file in cli_files[:5]:  # Limit for demo
            try:
                ...
                has_color = any(re.search(pattern, content) for pattern in color_patterns)
                has_alternative = any(term in content.lower() for term in ['alt', 'describe', 'label', 'announce'])
    ...
    def _test_text_readability(self) -> List[AccessibilityIssue]:
        ...
        for doc_file in doc_files[:10]:  # Limit for demo
            try:
                ...
                if len(complex_sentences) > len(sentences) * 0.3:  # >30% complex sentences
                    issues.append(AccessibilityIssue(
        ...
        for py_file in python_files:
            ...
            try:
                ...
                for print_stmt in print_statements:
                    ...
                    if not any(word in print_stmt.lower() for word in ['error', 'warning', 'info', 'success', 'status', 'result', 'loading', 'processing']):
    ...
    def _test_alternative_descriptions(self) -> List[AccessibilityIssue]:
        ...
        for viz_file in viz_files:
            try:
                ...
                has_plots = any(plot_func in content for plot_func in [
                    'plt.', 'matplotlib', 'seaborn', 'plotly', 'bokeh'
                ])

                has_alt_text = any(alt_term in content.lower() for alt_term in [
                    'alt', 'alternative', 'describe', 'caption', 'title', 'label'
                ])
    ...
    def generate_accessibility_report(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "automated_fixable": len([i for i in self.accessibility_issues if i.automated_fixable]),
    ...
    def _test_learnability(self) -> UsabilityTest:
        ...
        for cli_file in cli_files[:5]:
            try:
                ...
                if any(help_term in content for help_term in ['--help', '-h', 'help(', 'usage', 'argparse']):
                    help_indicators += 1

                # Look for examples
                if any(example_term in content.lower() for example_term in ['example', 'demo', 'tutorial']):
                    help_indicators += 1
    ...
    def _test_efficiency(self) -> UsabilityTest:
        ...
        for cli_file in cli_files[:5]:
            try:
                ...
                if any(shortcut in content for shortcut in ['--', '-', 'alias', 'shortcut']):
                    efficiency_indicators += 1

                # Look for batch processing
                if any(batch_term in content for batch_term in ['batch', 'bulk', 'multiple', 'all']):
                    efficiency_indicators += 1

                # Look for configuration files
                if any(config_term in content for config_term in ['config', 'settings', 'profile']):
                    efficiency_indicators += 1
        ...
        for cli_file in cli_files[:5]:
            try:
                ...
                for func_name in function_matches:
                    ...
                    if any(word in func_name.lower() for word in [
                        'run', 'start', 'stop', 'create', 'delete', 'update',
                        'get', 'set', 'show', 'list', 'help', 'config'
                    ]):
                        intuitive_names += 1

                if function_matches:
                    memorability_score += (intuitive_names / len(function_matches)) * 100
    ...
    def _test_error_prevention(self) -> UsabilityTest:
        ...
        for py_file in python_files:
            ...
            try:
                ...
                if any(validation_term in content for validation_term in [
                    'validate', 'check', 'verify', 'assert', 'isinstance', 'hasattr'
                ]):
                    validation_indicators += 1

                # Look for error handling
                if any(error_term in content for error_term in [
                    'try:', 'except:', 'raise', 'ValueError', 'TypeError'
                ]):
                    validation_indicators += 1
    ...
    def _test_satisfaction(self) -> UsabilityTest:
        ...
        for py_file in python_files:
            ...
            try:
                ...
                if any(progress_term in content for progress_term in [
                    'progress', 'tqdm', 'loading', 'status', 'percentage'
                ]):
                    satisfaction_indicators += 1

                # Look for informative output
                if any(info_term in content for info_term in [
                    'print', 'log', 'info', 'success', 'complete'
                ]):
                    satisfaction_indicators += 1
...
def run_accessibility_usability_testing():
    ...
    comprehensive_results = {
        ...
        "combined_metrics": {
            ...
            "critical_accessibility_issues": accessibility_report.get("severity_breakdown", {}).get("critical", 0)
        },
    ...
    if accessibility_report.get("total_issues", 0) > 0:
        ...
        print(f"  Compliance Score: {accessibility_report.get('compliance_score', 0):.1f}%")
    ...
    if usability_report.get("total_tests", 0) > 0:
        ...
        print(f"  Tests Passed: {usability_report.get('tests_passed', 0)}/{usability_report.get('total_tests', 0)}")

        # Show metric scores
        metric_scores = usability_report.get('metric_scores', {})
        for metric, data in metric_scores.items():
            status = "✓ PASS" if data['passed'] else "✗ FAIL"
            print(f"    {status} {metric.title()}: {data['score']:.1f}% (benchmark: {data['benchmark']:.1f}%)")

    # Combined assessment
    combined = comprehensive_results["combined_metrics"]
    overall_quality_score = (combined["accessibility_score"] + combined["usability_score"]) / 2

    print(f"\nOVERALL QUALITY ASSESSMENT:")
    print(f"  Combined Quality Score: {overall_quality_score:.1f}%")

    if overall_quality_score >= 80:
        print(f"  🎯 Excellent accessibility and usability!")
    elif overall_quality_score >= 60:
        print(f"  ⚠️  Good accessibility and usability with room for improvement")
    else:
        print(f"  🔧 Significant improvements needed in accessibility and usability")
    ...
    print(f"✅ Task 5.5 Accessibility and Usability Testing Complete!")
