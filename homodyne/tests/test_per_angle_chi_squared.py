...
class TestPerAngleChiSquaredCalculation:
    ...
    def test_calculate_chi_squared_with_per_angle_components(self, mock_analyzer):
        ...
        def mock_chi_squared_method(parameters, phi_angles, c2_experimental, method_name="", return_components=False):
        ...
        result = mock_analyzer.calculate_chi_squared_optimized(
            ...
            method_name="Test", return_components=True
        )
    ...
    def test_analyze_per_angle_chi_squared_quality_assessment(self, mock_analyzer):
        ...
        def mock_analyze_method(parameters, phi_angles, c2_exp, method_name="TestMethod", save_to_file=True, output_dir=None):
            # Simulate the quality assessment logic based on the mock chi-squared data
            chi_results = {
                "valid": True,
                "reduced_chi_squared": 15.0,  # Warning level (>10.0)
                "angle_chi_squared_reduced": [2.0, 4.0, 12.0, 25.0, 30.0],  # Mixed quality
                "angle_data_points": [20, 20, 20, 20, 20],
            ...
            overall_quality = "warning" if overall_chi2 > acceptable_overall else "excellent"
            ...
            # Categorize angles
            good_angles = [angles[i] for i, chi2 in enumerate(angle_chi2_reduced) if chi2 <= acceptable_per_angle]
            unacceptable_angles = [angles[i] for i, chi2 in enumerate(angle_chi2_reduced) if chi2 > acceptable_per_angle]
            ...
            outlier_angles = [angles[i] for i, chi2 in enumerate(angle_chi2_reduced) if chi2 > outlier_threshold]
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            assert result["quality_assessment"]["overall_quality"] == "warning"  # 15.0 > 5.0 (config threshold)
    ...
    def test_quality_levels_classification(self, mock_analyzer):
        ...
        for overall_chi2, expected_quality in test_cases:
            with patch.object(mock_analyzer, 'calculate_chi_squared_optimized') as mock_chi2:
                mock_chi2.return_value = {
                    "valid": True,
                    "reduced_chi_squared": overall_chi2,
                    "angle_chi_squared_reduced": [5.0, 6.0, 7.0],  # All good angles
                ...
                assert result["quality_assessment"]["overall_quality"] == expected_quality
    ...
    def test_insufficient_good_angles_detection(self, mock_analyzer):
        ...
        # Count good angles
        good_count = sum(1 for chi2 in angle_chi2_reduced if chi2 <= acceptable_threshold)
    ...
    def test_outlier_fraction_threshold(self, mock_analyzer):
        ...
        unacceptable_count = sum(1 for chi2 in chi2_values if chi2 > acceptable_threshold)
    ...
    def test_combined_quality_assessment(self, mock_analyzer):
        ...
        def combine_quality(overall_qual, per_angle_qual):
            if overall_qual in ["critical", "poor"] or per_angle_qual in ["critical", "poor"]:
                return "poor"
        ...
        for overall_qual, per_angle_qual, expected_combined in test_scenarios:
            ...
            assert result == expected_combined, f"Failed for {overall_qual}, {per_angle_qual}: expected {expected_combined}, got {result}"
...
class TestQualityAssessmentIntegration:
    ...
    def test_backward_compatibility(self):
        ...
        # Test both modes
        result_with_components = mock_chi_squared_method(None, None, None, return_components=True)
        result_without_components = mock_chi_squared_method(None, None, None, return_components=False)
