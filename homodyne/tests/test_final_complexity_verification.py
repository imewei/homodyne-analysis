...
class ComplexityAnalyzer:
    ...
    def analyze_package(self, package_path: Path) -> Tuple[List[FunctionComplexity], Dict]:
        ...
        # Generate statistics
        high_complexity_functions = [f for f in all_functions
                                   if f.complexity >= self.high_complexity_threshold]
...
def generate_complexity_report(package_path: Path = None) -> Dict:
    ...
    high_complexity_functions = [f for f in all_functions
                               if f.complexity >= analyzer.high_complexity_threshold]
...
def print_complexity_report(report: Dict):
    ...
    if high_complexity:
        ...
        for i, func in enumerate(high_complexity[:20], 1):  # Show top 20
            file_short = func['file_path'].split('/')[-1] if '/' in func['file_path'] else func['file_path']
            print(f"{i:<5} {func['complexity']:<11} {func['name']:<30} {file_short:<50}")
    ...
    for func_name, file_path in refactored_functions:
        ...
        if file_path_obj.exists():
            ...
            if target_func:
                ...
                print(f"  {func_name:30} | Complexity: {target_func.complexity:2d} | {status}")
            else:
...
def main():
    ...
    try:
        ...
        if report["target_met"]:
            ...
            print(f"❌ {report['analysis_summary']['high_complexity_count']} high-complexity functions remain")
            return False
