...
try:
    ...
    # Define dummy classes/functions to avoid Pylance errors
    class HomodyneCompleter:
        def method_completer(self, prefix, parsed_args):
            return []
        def config_files_completer(self, prefix, parsed_args):
            return []
        def output_dir_completer(self, prefix, parsed_args):
            return []
        def analysis_mode_completer(self, prefix, parsed_args):
            return []
...
class TestShellCompletion:
    ...
    def test_install_completion_bash(self):
        ...
        with patch('homodyne.cli_completion.ARGCOMPLETE_AVAILABLE', True), \
    ...
    def test_install_completion_argument(self):
        ...
        with patch('sys.argv', ['homodyne', '--install-completion', 'bash']):
            with patch('homodyne.run_homodyne.COMPLETION_AVAILABLE', True):
                with patch('homodyne.run_homodyne.install_shell_completion') as mock_install:
                    mock_install.return_value = 0
                    result = main()
                    assert result == 0
                    mock_install.assert_called_once_with('bash')
    
    def test_interactive_command_argument(self):
        """Test 'interactive' command argument parsing.""" 
        from homodyne.run_homodyne import main
        
        with patch('sys.argv', ['homodyne', 'interactive']):
            with patch('homodyne.run_homodyne.COMPLETION_AVAILABLE', True):
                with patch('homodyne.run_homodyne.start_interactive_mode') as mock_interactive:
        ...
        with patch('sys.argv', ['homodyne', 'interactive']):
            with patch('homodyne.run_homodyne.COMPLETION_AVAILABLE', False):
                with patch('builtins.print') as mock_print:
                    ...
                    # Check that helpful error message was printed
                    printed_args = [call[0][0] for call in mock_print.call_args_list]
                    assert any("cmd2" in msg for msg in printed_args)


@pytest.mark.skipif(not COMPLETION_AVAILABLE, reason="Interactive features not available")
class TestInteractiveMode:
    """Test interactive CLI mode functionality."""
    
    def test_interactive_cli_creation(self):
        """Test that interactive CLI can be created."""
        try:
            from homodyne.cli_completion import InteractiveHomodyneCLI
            cli = InteractiveHomodyneCLI()
            assert cli is not None
            assert hasattr(cli, 'do_run')
            assert hasattr(cli, 'do_config')
            assert hasattr(cli, 'do_method')
        except ImportError:
            pytest.skip("Interactive CLI dependencies not available")
    
    def test_interactive_commands_exist(self):
        """Test that all expected interactive commands exist."""
        try:
            from homodyne.cli_completion import InteractiveHomodyneCLI
            cli = InteractiveHomodyneCLI()
            
            # Check that essential commands exist
            commands = ['run', 'config', 'method', 'status', 'ls', 'create_config', 'help_guide']
            for cmd in commands:
                assert hasattr(cli, f'do_{cmd}'), f"Missing command: {cmd}"
                
            # Check that tab completion methods exist
            completers = ['run', 'config', 'method', 'create_config']
            for completer in completers:
                assert hasattr(cli, f'complete_{completer}'), f"Missing completer: {completer}"
...
class TestCLIReferenceCompliance:
    ...
    def test_completion_covers_all_main_arguments(self):
        ...
        for expected_method in expected_completions['method']:
            assert expected_method in methods, f"Method {expected_method} missing from completion"
    
    def test_interactive_mode_commands_comprehensive(self):
        """Test that interactive mode provides comprehensive command coverage."""
        if not COMPLETION_AVAILABLE:
        ...
        try:
            ...
            utility_commands = [
                ...
                'help_guide',   # Comprehensive help
            ]
            
            all_commands = workflow_commands + utility_commands
            
            for cmd in all_commands:
                assert hasattr(cli, f'do_{cmd}'), f"Interactive command missing: {cmd}"
