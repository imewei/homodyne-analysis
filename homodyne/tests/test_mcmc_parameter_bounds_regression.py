...
class TestMCMCParameterBoundsRegression:
        ...
        return {
            ...
            "parameter_space": {
                "bounds": [
                    ...
                    {"name": "alpha", "min": -1.6, "max": -1.5, "type": "uniform"},  # Narrow range, initial at boundary
    ...
    def config_with_safe_alpha(self):
        ...
        return {
            ...
            "parameter_space": {
                "bounds": [
                    {"name": "D0", "min": 15000, "max": 20000, "type": "log-uniform"},
                    {"name": "alpha", "min": -1.8, "max": -1.2, "type": "uniform"},  # Wider range, initial in center
                    {"name": "D_offset", "min": 0, "max": 5, "type": "uniform"},
                    {"name": "gamma_dot_t0", "min": 0.0, "max": 0.0, "type": "fixed"},
                    {"name": "beta", "min": 0.0, "max": 0.0, "type": "fixed"},
                    {"name": "gamma_dot_t_offset", "min": 0.0, "max": 0.0, "type": "fixed"},
    ...
    def test_mcmc_parameter_bounds_analysis_boundary_alpha(self):
        ...
        print(f"  Initial position: at max boundary (distance from center: {distance_from_center:.3f})")
    ...
    def test_mcmc_parameter_bounds_analysis_safe_alpha(self):
        ...
        print(f"  Initial position: exactly at center (distance from center: {distance_from_center:.3f})")
    ...
    def test_alpha_bounds_configuration_comparison(self, temp_directory):
        ...
        problematic_config = {
            "parameter_space": {
                "bounds": [
                    {"name": "alpha", "min": -1.6, "max": -1.5, "type": "uniform"}  # Width: 0.1
                ]
            },
        ...
        safe_config = {
            "parameter_space": {
                "bounds": [
                    {"name": "alpha", "min": -1.8, "max": -1.2, "type": "uniform"}  # Width: 0.6
                ]
            },
        ...
        assert abs(prob_width - 0.1) < 1e-10  # Very narrow (within floating point precision)
        ...
        assert abs(safe_width - 0.6) < 1e-10  # Much wider (within floating point precision)
        assert safe_alpha == safe_mid  # At center
        assert safe_bound["min"] < safe_alpha < safe_bound["max"]  # Within bounds
        
        print(f"✓ Problematic alpha bounds: width={prob_width:.1f}, initial at boundary")
    ...
    def test_mcmc_bounds_validation_edge_cases(self):
        ...
        bounds_centered = [{"name": "alpha", "min": -2.0, "max": -1.0, "type": "uniform"}]
        ...
        for bounds, initial, case_name in [
            ...
            assert within_bounds, f"Value {value} should be within bounds [{bound['min']}, {bound['max']}] for case {case_name}"
            ...
            if case_name == "centered":
                assert min_dist == 0.5, f"Centered case should be 0.5 from each boundary"
            else:
                assert min_dist == 0.0, f"Boundary cases should be at distance 0 from a boundary"
