...
class TestMCMCInitialParameterHandling:
    ...
    def test_mcmc_uses_classical_results_when_available(self):
        ...
        with patch('homodyne.optimization.mcmc.create_mcmc_sampler') as mock_create_sampler:
            mock_sampler = Mock()
            ...
            # Test parameters
            initial_params = [1000, -0.5, 100]  # Should be overridden by classical results
            phi_angles = np.array([0.0])
            c2_exp = np.random.random((1, 100, 100))
            
            # Run MCMC optimization
            result = run_mcmc_optimization(mock_analyzer, initial_params, phi_angles, c2_exp)
            ...
            mock_create_sampler.assert_called_once_with(mock_analyzer, mock_analyzer.config)
    ...
    def test_run_all_methods_parameter_chaining(self):
        ...
        with patch('run_homodyne.run_classical_optimization') as mock_classical, \
            ...
            mock_classical.assert_called_once_with(mock_analyzer, initial_params, phi_angles, c2_exp)
            
            # Verify MCMC was called with classical results as initial parameters
            mock_mcmc.assert_called_once_with(mock_analyzer, classical_best_params, phi_angles, c2_exp, None)
    ...
    def test_classical_results_storage_on_analyzer(self):
        ...
        mock_optimizer.run_classical_optimization_optimized.return_value = (classical_best_params, mock_result)
        ...
        with patch('run_homodyne.ClassicalOptimizer') as mock_optimizer_class:
            ...
            result = run_classical_optimization(mock_analyzer, initial_params, phi_angles, c2_exp)
            
            # Verify optimizer was created and called
            mock_optimizer_class.assert_called_once_with(mock_analyzer, mock_analyzer.config)
    ...
    def test_mcmc_parameter_initialization_logging(self):
        ...
        with patch('homodyne.optimization.mcmc.create_mcmc_sampler') as mock_create_sampler, \
            ...
            mock_sampler = Mock()
            mock_sampler.run_mcmc_analysis.return_value = {'trace': None, 'diagnostics': {}}
            mock_create_sampler.return_value = mock_sampler
    ...
    def test_mcmc_fallback_parameter_initialization_logging(self):
        ...
        with patch('homodyne.optimization.mcmc.create_mcmc_sampler') as mock_create_sampler, \
            ...
            log_calls = [call[0][0] for call in mock_log.info.call_args_list]
            assert any("Using provided initial parameters" in call for call in log_calls)
    ...
    def test_run_all_methods_with_classical_failure(self):
        ...
        with patch('run_homodyne.run_classical_optimization') as mock_classical, \
            ...
            mock_mcmc.assert_called_once_with(mock_analyzer, initial_params, phi_angles, c2_exp, None)
