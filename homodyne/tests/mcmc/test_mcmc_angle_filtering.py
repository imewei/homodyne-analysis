...
class TestMCMCAngleFilteringCore:
    ...
    def mock_mcmc_sampler(self, mock_config_for_mcmc):
        ...
        sampler.mcmc_config = mock_config_for_mcmc["optimization_config"][
            "mcmc_sampling"
        ]
    ...
    def test_mcmc_data_shape_with_filtering(
        self, mock_mcmc_sampler, test_phi_angles_mcmc
    ):
    ...
    def test_mcmc_model_building_with_filtering(
        self, mock_mcmc_sampler, test_phi_angles_mcmc
    ):
...
class TestMCMCAngleFilteringIntegration:
    ...
    def test_mcmc_sampler_creation_with_filtering(self):
        ...
        try:
            from homodyne.optimization.mcmc import (  # noqa: F401
                MCMCSampler,
                create_mcmc_sampler,
            )
...
class TestMCMCAngleFilteringPerformance:
    ...
    def test_mcmc_data_reduction_calculation(self):
        ...
        reduction_factor = (
            total_data_points / filtered_data_points
            if filtered_data_points > 0
            else float("inf")
        )
        reduction_percentage = (
            (1 - filtered_data_points / total_data_points) * 100
...
class TestMCMCAngleFilteringBackwardCompatibility:
    ...
    def test_mcmc_default_parameter_behavior(self):
        ...
        try:
            ...
            assert (
                not sig.parameters["filter_angles_for_optimization"].annotation
