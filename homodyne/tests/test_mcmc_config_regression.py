...
class TestMCMCConfigurationRegression:
    ...
    def test_simon_config_exact_reproduction(self):
        ...
        assert mcmc_config.get("draws", 1000) == 10000, "Should use Simon's configured draws=10000, not default 1000"
        assert mcmc_config.get("chains", 2) == 8, "Should use Simon's configured chains=8, not default 2"
        assert mcmc_config.get("tune", 500) == 1000, "Should use Simon's configured tune=1000, not default 500"
    ...
    def test_old_trace_file_detection(self):
        ...
        assert validate_trace_dimensions(correct_trace, config_chains, config_draws) == True
        
        # Validation should fail for old trace
        assert validate_trace_dimensions(old_trace, config_chains, config_draws) == False
    ...
    def test_config_path_hierarchy_regression(self):
        ...
        mcmc_config_path = config.get("optimization_config", {}).get("mcmc_sampling", {})
