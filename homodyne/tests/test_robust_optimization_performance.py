...
class MockAnalysisCorePerformance:
    ...
    def compute_c2_correlation_optimized(self, params, phi_angles):
        ...
        D_eff = D0 * time_delays[np.newaxis, :]**abs(alpha) + D_offset * time_delays[np.newaxis, :]
...
class TestRobustOptimizationPerformance:
    ...
    def test_initialization_performance(self, performance_mock_core):
        ...
        assert avg_init_time < 0.001, f"Initialization too slow: {avg_init_time:.4f}s"
    ...
    def test_parameter_bounds_performance(self, performance_mock_core):
        ...
        assert avg_bounds_time < 0.0001, f"Bounds extraction too slow: {avg_bounds_time:.4f}s"
    ...
    def test_chi_squared_computation_performance(self, performance_mock_core):
        ...
        for _ in range(50):
            chi_squared = optimizer._compute_chi_squared(test_params, phi_angles, c2_experimental)
        
        end_time = time.time()
        avg_chi_squared_time = (end_time - start_time) / 50
        
        # Chi-squared computation should be fast (< 5ms for 20x100 data)
        assert avg_chi_squared_time < 0.005, f"Chi-squared computation too slow: {avg_chi_squared_time:.4f}s"
    ...
    def test_theoretical_correlation_performance(self, performance_mock_core):
        ...
        for _ in range(30):
            c2_theory = optimizer._compute_theoretical_correlation(test_params, phi_angles)
    ...
    def test_scenario_generation_performance(self, performance_mock_core):
        ...
        for n_scenarios in scenario_counts:
            ...
            max_time = 0.002 * n_scenarios  # 2ms per scenario maximum
            assert scenario_time < max_time, f"Scenario generation too slow: {scenario_time:.4f}s for {n_scenarios} scenarios"
    ...
    def test_scaling_with_data_size(self):
        ...
        for n_angles, n_times in data_sizes:
            mock_core = MockAnalysisCorePerformance(PERFORMANCE_CONFIG, n_angles, n_times)
            assert RobustHomodyneOptimizer is not None, "RobustHomodyneOptimizer not available"
        ...
        for i in range(1, len(chi_squared_times)):
            size_ratio = (data_sizes[i][0] * data_sizes[i][1]) / (data_sizes[i-1][0] * data_sizes[i-1][1])
            time_ratio = chi_squared_times[i] / chi_squared_times[i-1]
            
            # Time ratio should not exceed size ratio by more than factor of 2
            assert time_ratio < 2 * size_ratio, f"Performance scaling poor: time ratio {time_ratio:.2f} vs size ratio {size_ratio:.2f}"
...
class TestRobustOptimizationBenchmarks:
    ...
    def test_classical_vs_robust_performance_comparison(self, performance_mock_core):
        ...
        classical_optimizer = ClassicalOptimizer(performance_mock_core, PERFORMANCE_CONFIG)
        
        # Robust optimizer
        assert RobustHomodyneOptimizer is not None, "RobustHomodyneOptimizer not available"
        robust_optimizer = RobustHomodyneOptimizer(performance_mock_core, PERFORMANCE_CONFIG)
        ...
        if CVXPY_AVAILABLE:
            ...
            if robust_params is not None and classical_success:
                assert robust_time < 10 * classical_time, f"Robust optimization too slow: {robust_time:.3f}s vs classical {classical_time:.3f}s"
...
class TestRobustOptimizationStressTests:
    ...
    def test_high_noise_performance(self):
        ...
        assert optimization_time < 60, f"High noise optimization too slow: {optimization_time:.3f}s"
        ...
        for params in extreme_params:
            try:
                ...
                if np.isfinite(chi_squared):
                    ...
                    logging.info(f"Chi-squared is infinite for extreme params {params} - this is expected")
                    assert chi_squared == float('inf'), f"Expected inf, got {chi_squared} for params {params}"
