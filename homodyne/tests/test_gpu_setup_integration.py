...
class TestGPUWrapperFunctionality:
        ...
        with patch('sys.argv', test_args):
            ...
            if "--method" in test_args:
                ...
                if method_idx < len(test_args):
                    ...
                    assert method in ["classical", "robust"], "Should detect unsupported method"
            ...
            with patch('sys.argv', test_args):
                if "--method" in test_args:
                    ...
                    if method_idx < len(test_args):
                        ...
                        assert method not in ["classical", "robust"], f"Should support {supported_method}"
    ...
    def test_platform_detection(self):
        ...
        if current_platform == "Linux":
            ...
            logger.info(f"Platform {current_platform} does not support GPU acceleration")
            gpu_supported = False
    ...
    def test_mcmc_sampler_gpu_intent_detection(self):
        ...
        try:
            ...
            with patch('logging.Logger.info') as mock_info:
                ...
                assert not sampler.use_jax_backend, "Should disable JAX backend for CPU mode"
                assert os.environ.get("JAX_PLATFORMS") == "cpu", "Should set JAX_PLATFORMS=cpu"
...
class TestGPUSetupScriptLogic:
    ...
    def test_nvidia_library_detection_logic(self):
        ...
        # Simulate the library detection logic from activate_gpu.sh
        try:
            import site
            site_packages = site.getsitepackages()[0]
            
            nvidia_libs = []
            lib_names = ["cublas", "cudnn", "cufft", "curand", "cusolver", "cusparse", 
    ...
    def test_ld_library_path_construction(self):
        ...
        for test_path in test_paths:
            ...
            clean_path = ":".join([p for p in test_path.split(":") if "/usr/local/cuda" not in p and p])
            
            if "/usr/local/cuda" in test_path:
                assert "/usr/local/cuda" not in clean_path, f"Should remove CUDA paths from: {test_path}"
            else:
                assert clean_path == test_path, f"Should preserve non-CUDA paths: {test_path}"
...
def test_integration_gpu_files_exist():
    ...
    for file_path in required_files:
        ...
        if file_path.endswith('.sh'):
            ...
            assert os.access(full_path, os.X_OK), f"Shell script not executable: {file_path}"
