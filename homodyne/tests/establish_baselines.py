...
def create_comprehensive_baselines() -> Dict[str, Any]:
    ...
    if valid_times:
        baselines['summary'] = {
            ...
            'avg_memory_usage': sum(valid_memory) / len(valid_memory) if valid_memory else 0,
...
def compare_with_existing_baselines(new_baselines: Dict[str, Any], existing_file: Path):
    ...
    for module_name in new_baselines.get('individual_modules', {}):
        if module_name in existing_baselines.get('individual_modules', {}):
            new_cold = new_baselines['individual_modules'][module_name].get('cold_import', {})
            old_cold = existing_baselines['individual_modules'][module_name].get('cold_import', {})
            ...
            if new_time is not None and old_time is not None:
                ...
                status = "📈" if change_ratio > 1.1 else "📉" if change_ratio < 0.9 else "📊"
                print(f"  {status} {module_name}: {old_time:.3f}s → {new_time:.3f}s ({change_percent:+.1f}%)")
    ...
    for metric in ['avg_import_time', 'max_import_time', 'avg_memory_usage']:
        ...
        if old_value > 0:
            ...
            status = "📈" if change_percent > 10 else "📉" if change_percent < -10 else "📊"
            print(f"  {status} {metric}: {old_value:.3f} → {new_value:.3f} ({change_percent:+.1f}%)")
