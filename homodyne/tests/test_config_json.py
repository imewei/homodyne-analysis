...
class TestParameterValidation:
    ...
    def test_parameter_names_consistency(self, dummy_config):
        ...
        bounds_params = [bound["name"] for bound in dummy_config["parameter_space"]["bounds"]]
    ...
    def test_parameter_bounds_validity(self, dummy_config):
        ...
        for bound in bounds:
            ...
            assert "min" in bound
            assert "max" in bound
            assert bound["min"] < bound["max"], f"Invalid bound for {bound['name']}: {bound['min']} >= {bound['max']}"
            ...
            if bound["name"] == "D0":
                ...
                assert bound["min"] > 0, "Reference shear rate gamma_dot_t0 must be positive"
            elif bound["name"] in ["alpha", "beta"]:
                # Power-law exponents should be reasonable
                assert bound["min"] >= -5.0 and bound["max"] <= 5.0, f"Power-law exponent {bound['name']} has unreasonable bounds"
    ...
    def test_initial_parameters_within_bounds(self, dummy_config):
        ...
        bounds = {bound["name"]: bound for bound in dummy_config["parameter_space"]["bounds"]}
    ...
    def test_parameter_units_consistency(self, dummy_config):
        ...
        if "units" in dummy_config["initial_parameters"]:
            ...
            assert len(units) == len(parameter_names), "Number of units must match number of parameters"
            ...
            if "gamma_dot_t0" in unit_dict:
                assert "s⁻¹" in unit_dict["gamma_dot_t0"] or "s-1" in unit_dict["gamma_dot_t0"]
...
class TestConfigurationSections:
    ...
    def test_required_sections_present(self, dummy_config):
        ...
        for section in required_sections:
            assert section in dummy_config, f"Required section '{section}' missing from configuration"
    ...
    def test_experimental_data_structure(self, dummy_config):
        ...
        for field in required_fields:
            assert field in exp_data, f"Required field '{field}' missing from experimental_data"
            assert isinstance(exp_data[field], str), f"Field '{field}' should be a string"
    ...
    def test_optimization_config_structure(self, dummy_config):
        ...
        valid_methods = ["Nelder-Mead", "Powell", "BFGS", "L-BFGS-B", "CG", "SLSQP"]
        for method in classical["methods"]:
            assert method in valid_methods, f"Unknown optimization method: {method}"
...
class TestParameterTypes:
    ...
    def test_parameter_type_specifications(self, dummy_config):
        ...
        for bound in bounds:
            if "type" in bound:
                valid_types = ["uniform", "log-uniform", "normal", "log-normal"]
                assert bound["type"] in valid_types, f"Invalid parameter type: {bound['type']}"
                
                # Log-uniform parameters should have positive bounds
                if bound["type"] == "log-uniform":
                    assert bound["min"] > 0, f"Log-uniform parameter {bound['name']} must have positive min bound"
                    assert bound["max"] > 0, f"Log-uniform parameter {bound['name']} must have positive max bound"
...
class TestConfigurationValidation:
    ...
    def test_frame_range_validation(self, dummy_config):
        ...
        min_frames = dummy_config.get("validation_rules", {}).get("frame_range", {}).get("minimum_frames", 5)
        frame_count = end_frame - start_frame
        assert frame_count >= min_frames, f"Frame count {frame_count} below minimum {min_frames}"
    ...
    def test_computational_parameters_validation(self, dummy_config):
        ...
        if "computational" in dummy_config["analyzer_parameters"]:
            ...
            if "num_threads" in comp:
                ...
                assert comp["num_threads"] <= 1024, "Number of threads seems unreasonably large"
            
            if "max_threads_limit" in comp:
                assert comp["max_threads_limit"] > 0, "Max threads limit must be positive"
                if "num_threads" in comp:
        ...
        if "output_settings" in dummy_config:
            ...
            if "plotting" in output:
                ...
                if "figure_size" in plotting:
                    ...
                    assert len(fig_size) == 2, "Figure size should have exactly 2 elements"
                    assert all(s > 0 for s in fig_size), "Figure size dimensions must be positive"
...
class TestJSONSchemaCompliance:
    ...
    def test_numeric_types_in_config(self, dummy_config):
        ...
        for value in initial_values:
            assert isinstance(value, (int, float)), f"Parameter value {value} is not numeric"
        
        # Check bounds
        for bound in dummy_config["parameter_space"]["bounds"]:
            assert isinstance(bound["min"], (int, float)), f"Bound min {bound['min']} is not numeric"
            assert isinstance(bound["max"], (int, float)), f"Bound max {bound['max']} is not numeric"
    ...
    def test_string_fields_in_config(self, dummy_config):
        ...
        exp_data = dummy_config["experimental_data"]
        string_fields = ["data_folder_path", "data_file_name", "phi_angles_path", "phi_angles_file"]
            ...
            if isinstance(obj, dict):
                for key, value in obj.items():
                    ...
                    if isinstance(value, bool):
                        # This is expected
                        pass
                    elif key.lower() in ['enabled', 'use_', 'include_', 'create_', 'cache_'] or \
                         key.lower().endswith('_enabled') or key.lower().startswith('use_'):
                        # These should probably be booleans
                        assert isinstance(value, bool), f"{current_path} should be boolean but is {type(value)}"
                    else:
...
class TestConfigErrorHandling:
    ...
    def test_invalid_parameter_values(self):
        ...
        invalid_configs = [
            ...
            {"analyzer_parameters": {"temporal": {"start_frame": 100, "end_frame": 50}}},
            # Negative diffusion coefficient
            {"initial_parameters": {"values": [-100.0]}},
            # Invalid bounds
            {"parameter_space": {"bounds": [{"name": "test", "min": 10.0, "max": 5.0}]}}
        ]
