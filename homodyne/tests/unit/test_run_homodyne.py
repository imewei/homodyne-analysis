...
class TestArgumentParsing:
    ...
    def test_contrast_and_offset_parameters(self):
        ...
        with patch('sys.argv', [
            'run_homodyne.py', '--plot-simulated-data',
            '--contrast', str(contrast), '--offset', str(offset)
        ]):
            with patch('homodyne.run_homodyne.plot_simulated_data') as mock_plot:
                with patch('homodyne.run_homodyne.setup_logging'):
                    with patch('homodyne.run_homodyne.print_banner'):
...
class TestRunAnalysis:
    ...
    def test_run_analysis_success_classical(self, mock_core_class, tmp_path):
        ...
        with patch('homodyne.run_homodyne.run_classical_optimization') as mock_classical_opt:
    ...
    def test_run_analysis_different_methods(self, mock_core_class, tmp_path):
        ...
        for method, expected_call in zip(methods, expected_calls):
            args = argparse.Namespace(
                config=config_path,
                output_dir=tmp_path,
                static_isotropic=False,
            ...
            with patch(f'homodyne.run_homodyne.{optimization_function_name}') as mock_opt:
...
class TestPlotSimulatedData:
    ...
    def test_plot_simulated_data_success(self, mock_core_class, tmp_path):
        ...
        mock_core = Mock()
        mock_core_class.return_value = mock_core
        mock_core.calculate_c2_single_angle_optimized.return_value = 1.0  # Return float instead of Mock
    ...
    @patch('homodyne.run_homodyne.HomodyneAnalysisCore')
    def test_plot_simulated_data_with_custom_parameters(self, mock_core_class, tmp_path):
...
class TestMainIntegration:
    ...
    def test_main_plot_simulated_data_mode(self, tmp_path):
        ...
        with patch('sys.argv', ['run_homodyne.py', '--plot-simulated-data', '--config', config_path]):
            with patch('homodyne.run_homodyne.plot_simulated_data') as mock_plot:
                with patch('homodyne.run_homodyne.setup_logging'):
                    with patch('homodyne.run_homodyne.print_banner'):
                        with patch('homodyne.run_homodyne.logging.getLogger'):
    ...
    def test_main_analysis_failure(self, tmp_path):
        ...
        with patch('sys.argv', ['run_homodyne.py', '--config', config_path]):
            with patch('homodyne.run_homodyne.run_analysis', side_effect=RuntimeError("Analysis failed")):
                with patch('homodyne.run_homodyne.setup_logging'):
                    with patch('homodyne.run_homodyne.print_banner'):
                        with patch('homodyne.run_homodyne.logging.getLogger'):
                            with patch('sys.exit') as mock_exit:
    ...
    def test_main_system_exit_preservation(self, tmp_path):
        ...
        with patch('sys.argv', ['run_homodyne.py', '--config', config_path]):
            with patch('homodyne.run_homodyne.run_analysis', side_effect=SystemExit(42)):
                with patch('homodyne.run_homodyne.setup_logging'):
                    with patch('homodyne.run_homodyne.print_banner'):
                        with patch('homodyne.run_homodyne.logging.getLogger'):
                            with pytest.raises(SystemExit) as exc_info:
...
class TestErrorHandling:
    ...
    def test_mutually_exclusive_mode_arguments(self):
        ...
        with patch('sys.argv', ['run_homodyne.py', '--static-isotropic', '--laminar-flow']):
            with patch('sys.stderr'):
                with pytest.raises(SystemExit) as exc_info:
                    main()
                assert exc_info.value.code == 2  # argparse error
