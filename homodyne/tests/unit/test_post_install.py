...
class TestShellCompletionSetup:
    ...
    def create_temp_venv(self):
        ...
        (venv_path / "etc" / "conda" / "deactivate.d").mkdir(parents=True, exist_ok=True)
    ...
    @patch('sys.prefix')
    def test_install_shell_completion_bash_success(self, mock_prefix, mock_detect_shell, mock_is_venv):
    ...
    @patch('homodyne.post_install.detect_shell_type')
    @patch('sys.prefix')
    def test_install_shell_completion_zsh_success(self, mock_prefix, mock_detect_shell, mock_is_venv):
    ...
    @patch('homodyne.post_install.detect_shell_type')
    @patch('sys.prefix')
    def test_install_shell_completion_fish_success(self, mock_prefix, mock_detect_shell, mock_is_venv):
    ...
    @patch('homodyne.post_install.detect_shell_type')
    @patch('sys.prefix')
    def test_install_shell_completion_file_write_error(self, mock_prefix, mock_detect_shell, mock_is_venv):
...
class TestGPUSetup:
    ...
    @patch('sys.prefix')
    def test_install_gpu_acceleration_success(self, mock_prefix, mock_is_conda, mock_is_venv, mock_platform):
...
class TestInteractiveSetup:
    ...
    def test_interactive_setup_all_yes(self):
        ...
        responses = ['y', '', 'y', 'y']  # shell yes, default shell, GPU yes, advanced yes
        with patch('builtins.input', side_effect=responses):
            with patch('homodyne.post_install.install_shell_completion', return_value=True):
                with patch('homodyne.post_install.install_gpu_acceleration', return_value=True):
                    with patch('homodyne.post_install.install_advanced_features', return_value=True):
                        with patch('platform.system', return_value='Linux'):
                            with patch('builtins.print'):
...
class TestMainFunction:
    ...
    def test_main_installation_failures(self):
        ...
        with patch('sys.argv', ['post_install.py']):
            with patch('homodyne.post_install.install_shell_completion', return_value=False):
                with patch('homodyne.post_install.install_gpu_acceleration', return_value=False):
                    with patch('builtins.print'):
...
class TestErrorHandling:
    ...
    def test_install_with_unexpected_errors(self):
        ...
        with patch('homodyne.post_install.is_virtual_environment', return_value=True):
            with patch('homodyne.post_install.detect_shell_type', side_effect=RuntimeError("Unexpected error")):
