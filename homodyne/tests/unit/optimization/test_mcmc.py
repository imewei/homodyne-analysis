...
class TestMCMCSamplerInitialization:
    ...
    def test_mcmc_sampler_init_basic(self, mock_analysis_core, mcmc_config):
        ...
        with patch('homodyne.optimization.mcmc.PYMC_AVAILABLE', True):
            ...
            assert sampler.mcmc_config == mcmc_config["optimization_config"]["mcmc_sampling"]
    ...
    def test_mcmc_sampler_init_with_jax(self, mock_analysis_core, mcmc_config):
        ...
        with patch('homodyne.optimization.mcmc.JAX_AVAILABLE', True):
            ...
            assert sampler.config["optimization_config"]["mcmc_sampling"]["use_jax"] is True
    ...
    def test_mcmc_sampler_init_pymc_unavailable(self, mock_analysis_core, mcmc_config):
        ...
        # Mock the global PYMC_AVAILABLE at the exact location where it's checked
        with patch('homodyne.optimization.mcmc.PYMC_AVAILABLE', False):
            with pytest.raises(ImportError, match="PyMC is required for MCMC sampling"):
...
class TestLazyImports:
    ...
    def test_lazy_import_pymc_failure(self):
        ...
        # Test the lazy import by directly importing the submodules and making them fail
        with patch('homodyne.optimization.mcmc.pm', None), \
             patch('builtins.__import__', side_effect=ImportError("PyMC not found")):
            # Call the function directly to test its failure path
            with pytest.raises(ImportError, match="PyMC dependencies not available"):
                _lazy_import_pymc()
    ...
    def test_lazy_import_jax_failure(self):
        ...
        # Test the lazy import by making the imports fail
        with patch('homodyne.optimization.mcmc.pmjax', None), \
             patch('builtins.__import__', side_effect=ImportError("JAX not found")):
            pmjax = _lazy_import_jax()
            assert pmjax is None
...
class TestPyMCModelConstruction:
    ...
    @pytest.mark.skipif(not MCMC_MODULE_AVAILABLE, reason="MCMC module not available")
    def test_model_construction_success(self, mock_analysis_core, mcmc_config, mock_pymc_components):
        """Test successful PyMC model construction."""
        mock_pm, mock_az, mock_pt, mock_shared = mock_pymc_components
        
        with patch('homodyne.optimization.mcmc.PYMC_AVAILABLE', True), \
             patch('homodyne.optimization.mcmc._lazy_import_pymc', return_value=mock_pymc_components):
            sampler = MCMCSampler(mock_analysis_core, mcmc_config)
...
class TestMCMCFactory:
    ...
    @pytest.mark.skipif(not MCMC_MODULE_AVAILABLE, reason="MCMC module not available")
    def test_create_mcmc_sampler_pymc_unavailable(self, mock_analysis_core, mcmc_config):
        """Test MCMC sampler creation when PyMC is unavailable."""
        # Mock the global PYMC_AVAILABLE at the exact location where it's checked
        with patch('homodyne.optimization.mcmc.PYMC_AVAILABLE', False):
            with pytest.raises(ImportError, match="PyMC is required for MCMC sampling"):
                create_mcmc_sampler(mock_analysis_core, mcmc_config)
...
class TestErrorHandling:
    ...
    def test_sampling_memory_error(self, mock_analysis_core, mcmc_config):
        ...
        with patch('homodyne.optimization.mcmc.PYMC_AVAILABLE', True):
            ...
            with patch.object(sampler, 'run_mcmc_analysis') as mock_sample:
                mock_sample.side_effect = MemoryError("Insufficient memory for sampling")
