...
class TestHomodyneAnalysisCoreInit:
    ...
    def test_init_with_config_file(self, tmp_path):
        ...
        config_data = {
            "analyzer_parameters": {
                "temporal": {"start_frame": 1, "end_frame": 100, "dt": 1e-3},
    ...
    def test_init_missing_config_file(self):
        ...
        with patch("homodyne.analysis.core.NUMBA_AVAILABLE", False):
            ...
            assert (
                analyzer.config["metadata"]["description"]
...
class TestStaticModeDetection:
    ...
    def test_is_static_mode_static_isotropic(self, mock_analyzer):
        ...
        with patch.object(
            HomodyneAnalysisCore, "is_static_mode", return_value=True
        ) as mock_method:
            result = mock_method()
    ...
    def test_is_static_mode_laminar_flow(self, mock_analyzer):
        ...
        with patch.object(
            HomodyneAnalysisCore, "is_static_mode", return_value=False
        ) as mock_method:
            result = mock_method()
    ...
    def test_is_static_parameters_zero_shear(self, mock_analyzer):
        ...
        with patch.object(
            HomodyneAnalysisCore, "is_static_parameters", return_value=True
        ) as mock_method:
    ...
    def test_is_static_parameters_nonzero_shear(self, mock_analyzer):
        ...
        with patch.object(
            HomodyneAnalysisCore, "is_static_parameters", return_value=False
        ) as mock_method:
            result = mock_method(params[3:])
            assert result is False
...
class TestParameterValidation:
    ...
    def test_get_effective_parameter_count_laminar(self):
        ...
        with patch.object(
            HomodyneAnalysisCore, "get_effective_parameter_count"
        ) as mock_method:
    ...
    def test_get_effective_parameters_static(self):
        ...
        with patch.object(
            HomodyneAnalysisCore, "get_effective_parameters"
        ) as mock_method:
...
class TestMathematicalOperations:
    ...
    def test_calculate_diffusion_coefficient_optimized(self, mock_analyzer_math):
        ...
        with patch.object(
            HomodyneAnalysisCore, "calculate_diffusion_coefficient_optimized"
        ) as mock_method:
    ...
    def test_calculate_shear_rate_optimized(self, mock_analyzer_math):
        ...
        with patch.object(
            HomodyneAnalysisCore, "calculate_shear_rate_optimized"
        ) as mock_method:
            mock_method.return_value = np.array([0.1, 0.2, 0.3])

            params = np.array(
                [1.0, 0.1, 0.1, 30.0]
            )  # shear_rate0, beta, shear_offset, phi0
            result = mock_method(params)
...
class TestDataLoading:
    ...
    def mock_analyzer_data(self):
        ...
        mock.config = {
            ...
            "analysis": {
                "phi_angles": [0, 45],
                "angle_ranges": [{"start": 0, "end": 10}, {"start": 40, "end": 50}],
            },
        }
        mock._data_cache = {}
        return mock
    ...
    def test_load_experimental_data_success(self, mock_analyzer_data):
        ...
        with patch.object(
            HomodyneAnalysisCore, "load_experimental_data"
        ) as mock_method:
...
class TestChiSquaredCalculation:
    ...
    def test_calculate_chi_squared_optimized(self, mock_analyzer_chi2):
        ...
        exp_data = (
            np.random.rand(3, 50, 50) + 1.0
        )  # Add 1 to avoid issues with correlation data

        with patch.object(
            HomodyneAnalysisCore, "calculate_chi_squared_optimized"
        ) as mock_method:
...
class TestConfigurationOverrides:
    ...
    def test_apply_config_overrides_simple(self):
        ...
        # Mock the method
        with patch.object(
            HomodyneAnalysisCore, "_apply_config_overrides"
        ) as mock_method:
