...
def mock_config():
    ...
    return {
        ...
        "output_settings": {
            "results_directory": "/tmp/test_plots"
        }
    }
...
class TestPlotConfiguration:
    ...
    @pytest.mark.skipif(not PLOTTING_MODULE_AVAILABLE, reason="Plotting module not available")
...
class TestCorrelationFunctionPlotting:
    ...
    @pytest.mark.skipif(not PLOTTING_MODULE_AVAILABLE, reason="Plotting module not available")
    def test_plot_correlation_functions_success(self, mock_config, mock_analysis_results, temp_output_dir):
    ...
    @pytest.mark.skipif(not PLOTTING_MODULE_AVAILABLE, reason="Plotting module not available")
    def test_plot_correlation_functions_multi_angle(self, mock_analysis_results, temp_output_dir):
    ...
    @pytest.mark.skipif(not PLOTTING_MODULE_AVAILABLE, reason="Plotting module not available")
    def test_plot_correlation_functions_residuals(self, mock_analysis_results, temp_output_dir):
        """Test correlation function plotting with residuals."""
        with patch('homodyne.plotting.plot_c2_heatmaps') as mock_plot:
            mock_plot.return_value = Mock(spec=matplotlib.figure.Figure)
            
            # Add residuals to mock data
            residuals = (mock_analysis_results["correlation_data"] - 
                        mock_analysis_results["theoretical_data"])
    ...
    @pytest.mark.skipif(not PLOTTING_MODULE_AVAILABLE, reason="Plotting module not available")  
    def test_plot_correlation_functions_log_scale(self, mock_analysis_results, temp_output_dir):
...
class TestParameterPosteriorPlotting:
    ...
    def test_plot_parameter_posteriors_corner_plot(self, mock_mcmc_trace, temp_output_dir):
    ...
    @pytest.mark.skipif(not PLOTTING_MODULE_AVAILABLE, reason="Plotting module not available")
    def test_plot_parameter_posteriors_trace_plots(self, mock_mcmc_trace, temp_output_dir):
...
class TestConvergenceDiagnostics:
    ...
    @pytest.mark.skipif(not PLOTTING_MODULE_AVAILABLE, reason="Plotting module not available")
    def test_plot_convergence_diagnostics_success(self, mock_diagnostics, temp_output_dir):
...
class TestDiagnosticSummaryPlots:
    ...
    @pytest.mark.skipif(not PLOTTING_MODULE_AVAILABLE, reason="Plotting module not available")
    def test_create_diagnostic_summary_plots_success(self, mock_config, mock_analysis_results, temp_output_dir):
    ...
    @pytest.mark.skipif(not PLOTTING_MODULE_AVAILABLE, reason="Plotting module not available")
    def test_create_diagnostic_summary_plots_per_angle(self, mock_config, mock_analysis_results, temp_output_dir):
    ...
    @pytest.mark.skipif(not PLOTTING_MODULE_AVAILABLE, reason="Plotting module not available")
    def test_create_diagnostic_summary_plots_failure(self, mock_config, temp_output_dir):
...
class TestPlottingErrorHandling:
    ...
    def test_matplotlib_backend_errors(self):
        ...
        with patch('matplotlib.pyplot.figure', side_effect=RuntimeError("Backend error")):
            with patch('homodyne.plotting.plot_c2_heatmaps') as mock_plot:
...
class TestPlottingIntegration:
    ...
    @pytest.mark.skipif(not PLOTTING_MODULE_AVAILABLE, reason="Plotting module not available")
    def test_end_to_end_plotting_workflow(self, mock_config, mock_analysis_results, temp_output_dir):
        ...
        with patch('homodyne.plotting.plot_diagnostic_summary') as mock_create:
            ...
            with patch('homodyne.plotting.plot_c2_heatmaps') as mock_corr:
                ...
                with patch('homodyne.plotting.plot_mcmc_corner') as mock_post:
                    ...
                    with patch('homodyne.plotting.plot_mcmc_convergence_diagnostics') as mock_diag:
