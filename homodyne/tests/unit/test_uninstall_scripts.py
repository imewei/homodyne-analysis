...
class TestCompletionFilesCleanup:
    ...
    def create_temp_venv_structure(self):
        ...
        for dir_path in [bash_completion_dir, conda_activate_dir, conda_deactivate_dir,
...
class TestInteractiveCleanup:
    ...
    def test_interactive_cleanup_yes_to_all(self):
        ...
        with patch('builtins.input', return_value='y'):
            with patch('homodyne.uninstall_scripts.cleanup_completion_files', return_value=['file1.sh']):
                with patch('homodyne.uninstall_scripts.cleanup_gpu_files', return_value=['gpu1.py']):
    ...
    def test_interactive_cleanup_selective(self):
        ...
        with patch('builtins.input', side_effect=responses):
            with patch('homodyne.uninstall_scripts.cleanup_completion_files', return_value=['comp.sh']):
                with patch('builtins.print'):
                    interactive_cleanup()
    ...
    def test_interactive_cleanup_no_files_found(self):
        ...
        with patch('builtins.input', return_value='y'):
            with patch('homodyne.uninstall_scripts.cleanup_completion_files', return_value=[]):
                with patch('homodyne.uninstall_scripts.cleanup_gpu_files', return_value=[]):
...
class TestArgumentParsing:
    ...
    def test_main_with_interactive_flag(self):
        ...
        with patch('sys.argv', ['uninstall_scripts.py', '--interactive']):
            with patch('homodyne.uninstall_scripts.interactive_cleanup'):
                with patch('homodyne.uninstall_scripts.is_virtual_environment', return_value=True):
                    main()
    
    def test_main_with_dry_run_flag(self):
        """Test main function with dry run flag."""
        with patch('sys.argv', ['uninstall_scripts.py', '--dry-run']):
            with patch('homodyne.uninstall_scripts.cleanup_completion_files', return_value=['file.sh']):
                with patch('homodyne.uninstall_scripts.cleanup_gpu_files', return_value=['gpu.py']):
                    with patch('homodyne.uninstall_scripts.is_virtual_environment', return_value=True):
                        with patch('builtins.print'):
                            main()
    ...
    def test_main_invalid_environment(self):
        ...
        with patch('sys.argv', ['uninstall_scripts.py']):
            with patch('homodyne.uninstall_scripts.is_virtual_environment', return_value=False):
                with patch('builtins.print'):
...
class TestMainFunction:
    ...
    def test_main_cleanup_with_files(self):
        ...
        with patch('sys.argv', ['uninstall_scripts.py']):
            with patch('homodyne.uninstall_scripts.cleanup_completion_files', return_value=[('Shell completion', 'file1.sh')]):
                with patch('homodyne.uninstall_scripts.cleanup_gpu_files', return_value=[('GPU setup', 'gpu.py')]):
                    with patch('homodyne.uninstall_scripts.is_virtual_environment', return_value=True):
                        with patch('builtins.print') as mock_print:
                            main()
    ...
    def test_main_cleanup_gpu_files_only(self):
        ...
        with patch('sys.argv', ['uninstall_scripts.py', '--dry-run']):
            with patch('homodyne.uninstall_scripts.cleanup_gpu_files', return_value=['gpu1.py', 'gpu2.sh']):
                with patch('homodyne.uninstall_scripts.is_virtual_environment', return_value=True):
                    with patch('builtins.print') as mock_print:
                        main()
