...
class TestGurobiIntegration:
    ...
    def test_gurobi_optimization_simple_quadratic(self):
        ...
        success, result = optimizer._run_gurobi_optimization(
            quadratic_objective, initial_params, bounds, 
            test_config["optimization_config"]["classical_optimization"]["method_options"]["Gurobi"]
        )
        ...
        assert hasattr(result, 'method') and result.method == "Gurobi-QP"  # type: ignore
    ...
    def test_gurobi_with_bounds_constraints(self):
        ...
        def objective_outside_bounds(params):
            x = params[0]
            return (x - 10)**2  # Optimum at x=10, but we'll constrain to [0,5]
        
        test_config = {
            "parameter_space": {
                "bounds": [
                    {"name": "x", "min": 0.0, "max": 5.0}
                ]
            }
        }
        ...
        if success:
            # Should find optimum at boundary x=5 (closest to true optimum x=10)
            assert hasattr(result, 'x'), "Result should have x attribute on success"
            assert 0.0 <= result.x[0] <= 5.0, "Solution should respect bounds"  # type: ignore
    ...
    def test_gurobi_uses_mcmc_bounds(self):
        ...
        mcmc_bounds_config = {
            "parameter_space": {
                "bounds": [
                    {"name": "D0", "min": 1.0, "max": 1000000.0, "type": "TruncatedNormal"},
                    {"name": "alpha", "min": -2.0, "max": 2.0, "type": "Normal"},
                    {"name": "D_offset", "min": -100, "max": 100, "type": "Normal"},
                    {"name": "gamma_dot_t0", "min": 1e-06, "max": 1.0, "type": "TruncatedNormal"},
                    {"name": "beta", "min": -2.0, "max": 2.0, "type": "Normal"},
        ...
        for i, (expected_min, expected_max) in enumerate(expected_bounds):
            ...
            assert actual_min == expected_min, f"Parameter {i} min bound mismatch: {actual_min} != {expected_min}"
            assert actual_max == expected_max, f"Parameter {i} max bound mismatch: {actual_max} != {expected_max}"
    ...
    def test_gurobi_bounds_consistency_static_mode(self):
        ...
        mcmc_bounds_config = {
            "parameter_space": {
                "bounds": [
                    ...
                    {"name": "D_offset", "min": -100, "max": 100},
                    {"name": "gamma_dot_t0", "min": 0.0, "max": 0.0},  # Fixed for static
                    {"name": "beta", "min": 0.0, "max": 0.0},          # Fixed for static
                    {"name": "gamma_dot_t_offset", "min": 0.0, "max": 0.0},  # Fixed for static
                    {"name": "phi0", "min": 0.0, "max": 0.0}           # Fixed for static
                ]
            }
        }
        ...
        for i, (expected_min, expected_max) in enumerate(expected_bounds):
            actual_min, actual_max = bounds[i]
            assert actual_min == expected_min, f"Static parameter {i} min bound mismatch"
            assert actual_max == expected_max, f"Static parameter {i} max bound mismatch"
