...
class TestRobustHomodyneOptimizer:
    ...
    def test_optimizer_initialization(self, mock_analysis_core, test_config):
        ...
        assert RobustHomodyneOptimizer is not None, "RobustHomodyneOptimizer not available"
    ...
    def test_create_robust_optimizer_factory(self, mock_analysis_core, test_config):
        ...
        assert create_robust_optimizer is not None, "create_robust_optimizer not available"
        assert RobustHomodyneOptimizer is not None, "RobustHomodyneOptimizer not available"
    ...
    def test_check_dependencies(self, mock_analysis_core, test_config):
        ...
        if CVXPY_AVAILABLE:
            assert optimizer.check_dependencies() == True
        else:
    ...
    def test_chi_squared_computation(self, mock_analysis_core, test_config, synthetic_data):
    ...
    def test_theoretical_correlation_computation(self, mock_analysis_core, test_config, synthetic_data):
        ...
        assert np.all(c2_theory > 0)  # Correlation functions should be positive
    
    def test_linearized_correlation_computation(self, mock_analysis_core, test_config, synthetic_data):
...
@pytest.mark.skipif(not ROBUST_OPTIMIZATION_AVAILABLE or not CVXPY_AVAILABLE,
...
class TestDistributionallyRobustOptimization:
    ...
    def test_wasserstein_dro_basic(self, mock_analysis_core, test_config, synthetic_data):
        ...
        result = optimizer._solve_distributionally_robust(
            theta_init=synthetic_data["true_params"] * 1.1,  # Start near true params
    ...
    def test_wasserstein_uncertainty_radius_effect(self, mock_analysis_core, test_config, synthetic_data):
...
class TestScenarioBasedRobustOptimization:
    ...
    def test_bootstrap_scenario_generation(self, mock_analysis_core, test_config, synthetic_data):
    ...
    def test_scenario_robust_basic(self, mock_analysis_core, test_config, synthetic_data):
    ...
    def test_scenario_count_effect(self, mock_analysis_core, test_config, synthetic_data):
...
class TestEllipsoidalRobustOptimization:
    ...
    def test_ellipsoidal_robust_basic(self, mock_analysis_core, test_config, synthetic_data):
    ...
    def test_ellipsoidal_gamma_effect(self, mock_analysis_core, test_config, synthetic_data):
...
class TestRobustOptimizationInterface:
    ...
    def test_run_robust_optimization_wasserstein(self, mock_analysis_core, test_config, synthetic_data):
    ...
    def test_run_robust_optimization_all_methods(self, mock_analysis_core, test_config, synthetic_data):
        ...
        for method, (optimal_params, info) in results.items():
            if optimal_params is not None:
                ...
                assert len(optimal_params) == 3
                assert np.all(np.isfinite(optimal_params))
            # Note: Some methods may fail in test environment, which is acceptable
    
    def test_invalid_method_error(self, mock_analysis_core, test_config, synthetic_data):
...
class TestClassicalOptimizerIntegration:
    ...
    def test_run_single_method_robust_wasserstein(self, mock_create_robust, mock_analysis_core, test_config):
        ...
        mock_robust_optimizer.run_robust_optimization.return_value = (
            ...
            {"method": "wasserstein", "status": "success", "final_chi_squared": 1.5}
        )
...
class TestRobustOptimizationConfiguration:
    ...
    def test_configuration_validation(self, test_config):
        ...
        required_fields = ["enabled", "uncertainty_model", "uncertainty_radius", "solver_settings"]
...
class TestRobustOptimizationPerformance:
    ...
    def test_small_datasets(self, test_config):
        ...
        small_core.c2_experimental = np.random.randn(2, 5)  # Only 5 time points  # type: ignore
...
class TestRobustOptimizationIntegration:
    ...
    def test_robust_optimization_with_logging(self, mock_analysis_core, test_config, caplog):
        ...
        optimization_logs = [msg for msg in log_messages if "robust optimization" in msg.lower()]
