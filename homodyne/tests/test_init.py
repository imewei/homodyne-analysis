...
class TestPackageImports:
    ...
    def test_all_exports_defined(self):
        ...
        for name in homodyne.__all__:
            assert hasattr(homodyne, name), f"Exported name '{name}' not found in module"
            ...
            if attr is not None:
                ...
                if not name.startswith("__"):  # Skip dunder attributes
                    ...
                    assert callable(attr) or isinstance(attr, type) or hasattr(attr, '__class__'), \
...
class TestOptionalDependencyHandling:
    ...
    def test_graceful_degradation_scipy(self):
        ...
        assert homodyne.ClassicalOptimizer is None or callable(homodyne.ClassicalOptimizer)
    ...
    def test_graceful_degradation_cvxpy(self):
        ...
        # Either available or gracefully set to None
        assert homodyne.RobustHomodyneOptimizer is None or callable(homodyne.RobustHomodyneOptimizer)
        assert homodyne.create_robust_optimizer is None or callable(homodyne.create_robust_optimizer)
    ...
    def test_graceful_degradation_pymc(self):
        ...
        assert homodyne.create_mcmc_sampler is None or callable(homodyne.create_mcmc_sampler)
...
class TestImportPerformance:
    ...
    def test_lazy_import_optimization(self):
        ...
        assert any(opt is not None for opt in [homodyne.ConfigManager, homodyne.HomodyneAnalysisCore])
