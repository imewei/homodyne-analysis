...
class TestMCMCFunctionBehavior:
    ...
    def test_mcmc_function_returns_sensible_dict_with_mock(self, dummy_config):
        ...
        with patch.dict('sys.modules', {'pymc': mock_pm}):
            ...
            assert isinstance(result['posterior_means'], dict), "Posterior means must be dict"
            
            for param_name in param_names:
                assert param_name in result['posterior_means'], f"Must have posterior mean for {param_name}"
                assert result['posterior_means'][param_name] == expected_means[param_name], \
            ...
            print("✓ MCMC function returns sensible dictionary with all required components")
            print(f"  - Contains {len(required_keys)} required keys")
            print(f"  - Posterior means for {len(result['posterior_means'])} parameters")
...
class TestChiSquaredRegression:
    ...
    def test_chi_squared_deterministic_with_fixed_seed(self):
        ...
        for i in range(n_angles):
            for j in range(n_time):
                for k in range(n_time):
                    ...
                    value = 1.0 + 0.5 * np.exp(-0.1 * abs(j - k)) + 0.01 * np.sin(i + j + k)
        ...
        for version in ['v40_simulation', 'updated_simulation', 'current_simulation']:
            # Calculate chi-squared with same seed
            chi_squared = calculate_mock_chi_squared(test_params, phi_angles, c2_experimental, FIXED_SEED)
        ...
        for version, chi_squared in results_list[1:]:
            ...
            rel_diff = abs_diff / abs(base_chi_squared) if abs(base_chi_squared) != 0 else abs_diff
        ...
        print("✓ Chi-squared regression test PASSED - all versions produce identical results")
    ...
    def test_numerical_reproducibility_with_seeds(self):
        ...
        def deterministic_calculation(seed, iterations=100):
            ...
            result = np.mean(data ** 2) + np.std(data) * np.sum(np.exp(-0.1 * np.arange(50)))
        ...
        print("✓ Numerical reproducibility test PASSED - calculations are deterministic with fixed seeds")
        ...
        def test_mcmc_available():
            with patch.dict('sys.modules', {'pymc': mock_pm}):
                ...
                result = {
                    'trace': mock_trace,
                    'time': 1.234,
                    'posterior_means': {name: 50.0 + i * 5.0 for i, name in enumerate(param_names)},
