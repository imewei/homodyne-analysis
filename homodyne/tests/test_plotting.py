...
class TestDiagnosticPlots:
    ...
    def test_plot_diagnostic_summary_comprehensive_all_subplots(
        ...
        try:
            ...
            try:
                ...
                if (
                    ...
                    rhat_data = xr.Dataset(
                        {
                            # Scalar value, no dims needed
                            param: xr.DataArray(1.01)
                            for param in param_names
                        }
                    )
                    ess_data = xr.Dataset(
            ...
            results = {
                ...
                # For Plot 2: Parameter uncertainties (computed from MCMC
                # trace)
                "mcmc_trace": trace_data,
                # For Plot 3: MCMC convergence diagnostics (ArviZ Dataset
                # format)
                "mcmc_diagnostics": {
    ...
    def test_plot_diagnostic_summary_minimal_data_with_placeholders(
        ...
        assert (
            file_size > 10000
        ), "Plot file should contain placeholder messages"
...
class TestMCMCPlots:
    ...
    def test_plot_mcmc_convergence_diagnostics(
        ...
        try:
            ...
            plot_files = list(
                temp_directory.glob("mcmc_convergence_diagnostics.png")
            )
            assert len(plot_files) == 1
    ...
    def test_plot_mcmc_convergence_diagnostics_arviz_format(
        ...
        try:
            ...
            # Create mock trace data with realistic parameter names for static
            # isotropic mode
            n_chains, n_draws = 4, 1000
            param_names = ["D0", "alpha", "D_offset"]
    ...
    def test_create_all_plots_complete(
        ...
        for plot_type in expected_plots:
            if plot_type in plot_status:
                # If the plot was attempted, it should have succeeded with
                # dummy data
                assert (
    ...
    def test_create_all_plots_with_mcmc_data(
        ...
        try:
            ...
            mcmc_results = {
                ...
                "best_parameters": {
                    name: np.random.normal() for name in param_names
                },
                ...
                "mcmc_diagnostics": {
                    ...
                    "ess_bulk": {
                        name: np.random.randint(400, 1000)
                        for name in param_names
                    },
                    "mcse_mean": {
...
class TestScalingOptimization:
    ...
    def test_residual_calculation_with_fitted_values(
        ...
        exp = (
            theory * contrast
            + offset
            + 0.1 * np.random.randn(n_angles, n_t2, n_t1)
        )
