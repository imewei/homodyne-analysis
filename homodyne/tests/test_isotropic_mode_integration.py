...
class TestIsotropicModeIntegration:
    ...
    def isotropic_config(self):
        ...
        return {
            ...
            "initial_parameters": {
                "values": [1000.0, 0.0, 100.0, 0.0, 0.0, 0.0, 0.0]
            },
            "analysis_settings": {
                "static_mode": True,
    ...
    def test_isotropic_mode_optimization_workflow(
        ...
        n_times = 49
        mock_c2_data = np.exp(-np.linspace(0, 5, n_times)).reshape(
            1, -1
        ) + 0.1 * np.random.rand(1, n_times)
        mock_load.return_value = {"c2_exp": mock_c2_data}
    ...
    def test_isotropic_mode_correlation_calculation(
        ...
        params = np.array(
            [1000.0, 0.0, 100.0, 999.0, 999.0, 999.0, 999.0]
        )  # Large values for last 4
        ...
        # Reasonable upper bound for normalized C2
    ...
    def test_isotropic_mode_chi_squared_calculation(
        ...
        mock_exp_data = np.exp(-np.linspace(0, 3, 20)) + 0.05 * np.random.rand(
            20
        )
    ...
    def test_isotropic_mode_caching_behavior(
        ...
        # Mock experimental data for single angle
        mock_c2_data = np.random.rand(
            1, 49, 49
        )  # Single angle, 49x49 correlation matrix
    ...
    def test_isotropic_vs_anisotropic_mode_comparison(self, temp_directory):
        ...
        assert (
            iso_manager.is_angle_filtering_enabled() is False
        )  # Disabled in isotropic
        assert (
            aniso_manager.is_angle_filtering_enabled() is True
        )  # Enabled in anisotropic
    ...
    def test_isotropic_mode_end_to_end_workflow(
        ...
        with patch(
            "homodyne.analysis.core.os.path.exists", return_value=False
        ):
            with patch("homodyne.analysis.core.np.load") as mock_load:
                with patch(
                    "homodyne.analysis.core.np.savez_compressed"
                ) as mock_savez:
                    ...
                    with patch.object(core, "_load_raw_data") as mock_load_raw:
                        ...
                        assert (
                            core.config_manager.is_static_isotropic_enabled()
                        )
                        assert (
                            not core.config_manager.is_angle_filtering_enabled()
                        )
                        assert core.get_effective_parameter_count() == 3
