...
def assert_mcmc_config_values(sampler, expected_draws=None, expected_chains=None, expected_tune=None):
    ...
    if expected_draws is not None:
        ...
        assert actual_draws == expected_draws, f"Expected draws={expected_draws}, got {actual_draws}"
        
    if expected_chains is not None:
        actual_chains = sampler.mcmc_config.get("chains", 2)
        assert actual_chains == expected_chains, f"Expected chains={expected_chains}, got {actual_chains}"
        
    if expected_tune is not None:
        actual_tune = sampler.mcmc_config.get("tune", 500)
        assert actual_tune == expected_tune, f"Expected tune={expected_tune}, got {actual_tune}"
