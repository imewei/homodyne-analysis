...
def timestamped_filename(base_name: str, chi2: Optional[float] = None, 
    ...
    if config and "output_settings" in config and "file_naming" in config["output_settings"]:
        naming_config = {**default_naming, **config["output_settings"]["file_naming"]}
    else:
...
def save_numpy(data: np.ndarray, filepath: Union[str, Path], 
               compressed: bool = True, **kwargs) -> bool:
    """
...
def save_pickle(data: Any, filepath: Union[str, Path], protocol: int = pickle.HIGHEST_PROTOCOL,
                **kwargs) -> bool:
    """
...
def save_fig(figure, filepath: Union[str, Path], dpi: int = 300, 
...
def get_output_directory(config: Optional[Dict] = None) -> Path:
    ...
    if config and "output_settings" in config:
        ...
        logger.warning("No output directory configuration found, using default: ./results")
    
    return ensure_dir(output_dir)


def save_analysis_results(results: Dict, config: Optional[Dict] = None, 
                         base_name: str = "analysis_results") -> Dict[str, bool]:
    """
    ...
    if "correlation_data" in results and isinstance(results["correlation_data"], np.ndarray):
    ...
    if any(key.startswith("mcmc_") or key.startswith("bayesian_") for key in results.keys()):
